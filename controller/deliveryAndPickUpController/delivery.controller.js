                    ...
                    if (token) {
                        ...
                        if (!deliveryData.deliveryId || !deliveryData.payTypeData ||
                            ...
                            let sql_query_chkOfficial = `SELECT billId FROM billing_Official_data WHERE billId = '${deliveryData.payTypeData.billId}';
                                                             ...
                                                             CROSS JOIN (SELECT COALESCE(resetDate, '04-01') AS resetDate FROM billing_firm_data WHERE firmId = (SELECT firmId FROM billing_category_data WHERE categoryName = '${deliveryData.payTypeData.deliveryType}') LIMIT 1) AS frm
                                                             WHERE bod.firmId = (SELECT firmId FROM billing_category_data WHERE categoryName = '${deliveryData.payTypeData.deliveryType}')
