    ...
    try {
        ...
        if (!billId) {
            ...
            let sql_queries_getDetails = `SELECT
                                          ...
                                          WHERE bst.billId = '${billId}' AND bst.tokenStaus != 'cancelled'
            ...
            pool.query(sql_queries_getDetails, (err, data) => {
                if (err) {
                    ...
                    // Remove empty item groups (where all items were cancelled)
                    const filteredResult = result.filter(group => group.items.length > 0);
            ...
            if (err) {
                ...
                pool.query(sql_query_getBillData, (err, billData) => {
                    if (err) {
                        ...
                        req?.io?.emit('updateTableView');
                        req?.io?.emit('print_Bill_123', json);
