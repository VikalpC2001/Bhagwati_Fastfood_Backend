    ...
    try {
        ...
        pool.query(sql_querry_getCountDetails, (err, rows, fields) => {
            if (err) {
                ...
                const sql_query_getDetails = `SELECT addonsId ,groupId, addonsName, price, isActive FROM item_addons_data LIMIT ${limit}`;
                ...
                if (!data.groupId || !data.addonsName || !data.price) {
                    ...
                    pool.query(`SELECT addonsName FROM item_addons_data WHERE addonsName = '${data.addonsName}'`, function (err, row) {
                        ...
                        if (row && row.length) {
                            ...
                            const sql_querry_addCategory = `INSERT INTO item_addons_data (addonsId, groupId, addonsName, price, isActive)  
                                                            VALUES ('${addonsId}', '${data.groupId}', '${data.addonsName}', ${data.price}, ${data.isActive})`;
