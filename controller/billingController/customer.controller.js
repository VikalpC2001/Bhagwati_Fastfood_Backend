...
const getStaticsByCustomer = (req, res) => {
    try {
        ...
        const sql = `SELECT
                         ...
                         COALESCE(SUM(CASE WHEN bd.billType = 'Dine In' AND bd.billStatus <> 'Cancel' THEN 1 ELSE 0 END), 0) AS totalDineInOrders,
                         COALESCE(SUM(CASE WHEN bd.billType = 'Dine In' AND bd.billStatus <> 'Cancel' THEN bd.settledAmount ELSE 0 END), 0) AS totalDineInPayment,
        ...
        pool.query(sql, params, (err, data) => {
            ...
            const json = {
                ...
                "DineIn Summary": data[0].totalDineInOrders + ' | ₹ ' + parseFloat(data[0].totalDineInPayment ? data[0].totalDineInPayment : 0).toLocaleString('en-IN'),
...
async function createPDFList(res, datas, sumFooterArray, tableHeading, fileName = 'customer-bills.pdf') {
    try {
        const doc = new jsPDF();
...
const exportPdfForBillDataByCustomerId = (req, res) => {
    try {
        const customerId = req.query.customerId || null;
        const startDate = req.query.startDate ? req.query.startDate.slice(4, 15) : null;
        const endDate = req.query.endDate ? req.query.endDate.slice(4, 15) : null;
        ...
        pool.query('SELECT customerName FROM billing_customer_data WHERE customerId = ?', [customerId], (err, customerRows) => {
            if (err) {
            ...
            let dataSql = `SELECT
                                ...
                                END AS "Token No.",
                                bd.cashier AS "cashier",
                                bd.billType AS "Bill Type",
                                bd.billPayType AS "Pay Type",
                                bd.totalAmount AS "Amount",
                                bd.totalDiscount AS "Discount",
                                bd.settledAmount AS "Settled Amount",
                                DATE_FORMAT(bd.billDate, '%d-%m-%Y') AS "Date",
                                DATE_FORMAT(bd.billCreationDate, '%h:%i %p') AS "Time"
                            FROM billing_data bd
                            LEFT JOIN billing_token_data AS btd ON btd.billId = bd.billId
                            LEFT JOIN billing_billwisecustomer_data bwc ON bd.billId = bwc.billId
                            WHERE bwc.customerId = ?
            ...
            pool.query(dataSql, dataParams, (err, billRows) => {
                if (err) {
                    console.error('Data Query Error:', err);
                    return res.status(500).send('Database Error');
                }
                ...
                const rows = JSON.parse(JSON.stringify(billRows));
                const keys = Object.keys(rows[0]);
                const sumTotalAmount = rows.reduce((s, r) => s + (Number(r["Amount"]) || 0), 0);
                const sumTotalDiscount = rows.reduce((s, r) => s + (Number(r["Discount"]) || 0), 0);
                const sumSettledAmount = rows.reduce((s, r) => s + (Number(r["Settled Amount"]) || 0), 0);

                const sumFooterArray = new Array(1 + keys.length).fill('');
                sumFooterArray[0] = 'Total';
                const idxAmount = keys.indexOf('Amount');
                const idxDiscount = keys.indexOf('Discount');
                const idxSettled = keys.indexOf('Settled Amount');
                if (idxAmount !== -1) sumFooterArray[1 + idxAmount] = sumTotalAmount;
                if (idxDiscount !== -1) sumFooterArray[1 + idxDiscount] = sumTotalDiscount;
                if (idxSettled !== -1) sumFooterArray[1 + idxSettled] = sumSettledAmount;

                const fromLabel = startDate && endDate ? `${startDate} To ${endDate}` : `Year ${new Date().getFullYear()}`;
                const tableHeading = `Bill Data From ${fromLabel}`;
                const fileName = `customer-bills-${customerId}-${Date.now()}.pdf`;
    ...
    try {
        ...
        if (!word || word.length < 3) {
            ...
            const sql = `SELECT
                         ...
                         FROM billing_customer_data bcd
...
module.exports = {
    ...
    exportPdfForBillDataByCustomerId
}